...
/**
 * Unit tests for {@link VehicleController}. Uses Mockito and MockMvc to
 * simulate HTTP requests and validate controller behavior.
 */
...
public class VehicleControllerTest implements WebMvcConfigurer {
	...
	/**
	 * Test for creating a vehicle. - Simulates POST /api/vehicles - Mocks
	 * vehicleService.createVehicle() - Verifies status 200 OK and response body
	 */
	@Test
	...
	/**
	 * Test for retrieving all vehicles. - Simulates GET /api/vehicles - Mocks
	 * vehicleService.getAllvehicles() - Verifies list size and content
	 */
	@Test
	...
	/**
	 * Test for retrieving a vehicle by ID (success case). - Simulates GET
	 * /api/vehicles/{id} - Verifies status 200 OK and response content
	 */
	@Test
	void getVehicleById_ShouldReturnVehicleDTO_WhenVehicleExists() throws Exception {
		...
		mockMvc.perform(get("/api/vehicles/1").contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk())
				.andExpect(jsonPath("$.vehicleNumber").value("ABC1234"))
				.andExpect(jsonPath("$.model").value("Model X"));
	...
	/**
	 * Test for retrieving a vehicle by ID (not found case). - Simulates GET
	 * /api/vehicles/{id} - Verifies status 404 Not Found
	 */
	@Test
	void testGetVehicleById_NotFound() throws Exception {
		when(vehicleService.getVehicleById(1L)).thenReturn(null);

		mockMvc.perform(get("/api/vehicles/1")).andExpect(status().isNotFound());
	}

	/**
	 * Test for deleting a vehicle by ID. - Simulates DELETE /api/vehicles/{id} -
	...
	/**
	 * Test for searching vehicles based on filters. - Simulates POST
	 * /api/vehicles/search - Verifies status 200 OK and returned list
	 */
	@Test
	void searchVehicles_ShouldReturnListOfMatchingVehicles() throws Exception {
		...
		mockMvc.perform(post("/api/vehicles/search").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(vehicleDTO))).andExpect(status().isOk())
	 ...
	 * Simulates PUT /api/vehicles/{id}/custom-query-update - Verifies status 200 OK
	 * and success message
	 */
	@Test
	void updateSelectedFields_ShouldReturnSuccessMessage_WhenVehicleIsUpdated() throws Exception {
		...
		mockMvc.perform(
				put("/api/vehicles/1/custom-query-update").param("model", "Model S").param("fuelType", "Electric"))
				.andExpect(status().isOk()).andExpect(content().string("Vehicle updated successfully"));
	...
	void testUpdateSelectedFields_Failure() throws Exception {
		...
		mockMvc.perform(
				put("/api/vehicles/1/custom-query-update").param("model", "ModelX").param("fuelType", "Electric"))
				.andExpect(status().isOk()).andExpect(content().string("vehicle id not found"));
